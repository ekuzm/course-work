...
void ProjectService::addTaskToProject(Company* company, int projectId, const Task& task) {
    if (!company) {
...
void ProjectService::recomputeProjectTotals(Company* company, int projectId) {
    if (!company) {
        return;
    }
    Project* project = company->getProject(projectId);
