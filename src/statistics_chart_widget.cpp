...
StatisticsChartWidget::StatisticsChartWidget(QWidget* parent) : QWidget(parent), animationProgress(0.0) {
...
void StatisticsChartWidget::paintEvent(QPaintEvent* event) {
    ...
    if (employees.empty()) {
        ...
        painter.drawText(0, 0, width, height, Qt::AlignCenter, "No employees data available");
        return;
    }
    ...
    if (employeeData.empty()) {
        painter.setPen(QPen(QColor(150, 150, 150), 1));
        painter.setFont(QFont("Segoe UI", 16));
        painter.drawText(0, 0, width, height, Qt::AlignCenter, "No active employees");
        return;
    }
...
void StatisticsChartWidget::drawMainEmployeeSalaryChart(
    ...
    const std::vector<std::pair<std::shared_ptr<Employee>, double>>& employeeData, double progress) {
    int padding = 60;
    int chartX = padding;
    ...
    painter.drawText(legendX, legendY, 150, 20, Qt::AlignLeft, "Employee Types:");
    ...
    for (const auto& [type, color] : fixedTypeColors) {
        ...
        painter.setPen(QPen(QColor(66, 66, 66), 1));
        painter.setFont(QFont("Segoe UI", 9));
        painter.drawText(legendX + 20, legendY, 130, 15, Qt::AlignLeft | Qt::AlignVCenter, type);
        legendY += 20;
    }
}
